...
Usage (CLI):
  python toolkit/runtime_main.py path/to/agent.md
...
def run_skill_runner(skill_name: str, agent_id: str = None) -> Dict[str, Any]:
    """Resolve a skill's runner and execute it. Returns dict with `name`,
    ...
    if runner_path is None or not runner_path.exists():
        return {
            "name": skill_name,
            "exit_code": 2,
            "output": f"No runner for {skill_name} (tried {runner_path})",
        }
...
def main():
    if len(sys.argv) < 2:
        print("Usage: toolkit/runtime_main.py path/to/agent.md")
        sys.exit(2)
