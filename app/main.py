...
def handle_user_exists(request, exc: UserAlreadyExists):
    return JSONResponse(status_code=409, content={"error": "user_already_exists", "message": str(exc)})
...
def handle_repo_error(request, exc: RepositoryError):
    return JSONResponse(status_code=500, content={"error": "repository_error", "message": str(exc)})
...
@app.api_route("/users", methods=["GET", "POST", "PUT", "PATCH", "DELETE"], include_in_schema=False)
def users_redirect():
    return RedirectResponse(url="/api/v1/users")


@app.api_route("/users/{full_path:path}", methods=["GET", "POST", "PUT", "PATCH", "DELETE"], include_in_schema=False)
